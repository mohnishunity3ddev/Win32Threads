#if !defined(GLTHREAD_H)

// Glue Based Double linked list.
#include "defines.h"

// NOTE: There is no data pointer in this linked list node since this is Glue
// Based. Whatever APPDATA we have, the strucvt for it will have the node of
// this glthread. the glthread itself does not hold any data. The Data holds
// this node. And it uses this node to stick itself to other APPDATA structs.
...
#define GLTHREAD_TO_STRUCT(fn_name, structure_name, field_name)                \
    ...
    {                                                                          \
        return (                                                               \
            structure_name *)((char *)(glthreadptr) -                          \
