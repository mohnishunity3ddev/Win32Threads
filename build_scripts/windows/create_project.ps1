...
if($null -ne $foundChapterDir) {
    ...
    Add-Content -Path .\src\$projNameArg.cpp -Value "#include <windows.h>`n`n#include <defines.h>`n#include <Logger/Win32Logger.h>`n`ni32 CALLBACK`nWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR pCmdLine,`n          i32 CmdShow)`n{`n    Win32Logger::LogInfoUnformatted(`"Hello, World!`");`n    pause();`n    FreeConsole();`n    return 0;`n}"
